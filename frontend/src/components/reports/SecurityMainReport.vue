...
export default {
  ...
  computed: {
    securityTeamId() {
      return this.$store.state.securityGroup.currentGroup?.id
    },

    reportData() {
      return this.$store.state.securityReport.reportBySecurityTeamData
    }
  },

  methods: {
    getReportBySecurityTeam() {
      this.$store.dispatch('securityReport/getReportBySecurityTeam', this.securityTeamId)
    }
  },

  mounted() {
    if (this.securityTeamId) this.getReportBySecurityTeam()
  },

  watch: {
    securityTeamId(value) {
      if (value) this.getReportBySecurityTeam()
    }
  }
}
</script>
